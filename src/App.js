...
const CASES = {
  serviceops: {
    ...
    sections: [
      ...
      {
        h: 'System Overview',
        p: 'The platform models the service lifecycle end-to-end: intake → assignment → execution → QA → completion, with approvals, escalations, role-based controls, and audit history.',
      },
      {
  ...
  umare: {
    ...
    sections: [
      {
        h: 'Problem Context',
        p: 'Manual incident handling doesn’t scale. Response quality varies by operator, resolution depends on tribal knowledge, and teams grow headcount instead of fixing root causes.',
      },
      {
        h: 'System Overview',
  ...
  hyperflow: {
    title: 'HyperFlow ƒ?" Workflow Execution Engine',
    ...
    sections: [
      {
        h: 'Problem Context',
        p: 'Operational workflows live in people’s heads and documents, making exceptions expensive and scaling dependent on coordination roles.',
      },
      {
        h: 'System Overview',
  ...
  cortex: {
    title: 'Cortex ƒ?" Ops Intelligence Dashboard',
  ...
  gbp: {
    title: 'GBP Assistant ƒ?" Local SEO Automation Platform',
...
function MainApp() {
  ...
  useEffect(() => {
    ...
    // Add ServiceOps live demo link on card
    const serviceCard = nodes.scope?.querySelector('#systems .panel');
    if (serviceCard && !serviceCard.querySelector('.panelActions')) {
      const actions = document.createElement('div');
      actions.className = 'panelActions';
      actions.innerHTML =
        '<a class="ghostLink" href="https://serviceops.pro/" target="_blank" rel="noreferrer noopener" aria-label="Open ServiceOps live demo">Live Demo →</a>';
      const bullets = serviceCard.querySelector('.bullets');
      if (bullets) {
        bullets.insertAdjacentElement('afterend', actions);
      }
    }
    ...
    const panelKeys = ['serviceops', 'umare', 'hyperflow', 'cortex', 'gbp'];
